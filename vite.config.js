...
function migrateOptimizeDepsConfig (config) {
    ...
    if (config.environments) {
        nextConfig.environments = Object.fromEntries(
            Object.entries(config.environments).map(([name, environment]) => [
                name,
                migrateOptimizeDepsConfig(environment),
            ])
        )
    }
...
function migrateOptimizeDepsOptions (optimizeDeps) {
    ...
    if (esbuildOptions.loader !== undefined) {
        ...
        for (const [key, value] of Object.entries(esbuildOptions.loader)) {
            if (rolldownOptions.moduleTypes[key] === undefined &&
                !UNSUPPORTED_MODULE_TYPES.has(value)) {
...
function wrapConfigHook (hook) {
    if (!hook) return hook

    return function wrappedConfigHook (...args) {
        return Promise.resolve(hook.apply(this, args))
            .then(result => migrateOptimizeDepsConfig(result))
    }
}

function wrapCloudflarePluginsForVite8 (plugins) {
    return plugins.map(plugin => ({
        ...plugin,
        config: wrapConfigHook(plugin.config),
